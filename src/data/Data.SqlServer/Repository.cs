    ...
    where TEntity : class
{
    ...
    public IQueryable<TEntity> GetPartitionQueryable(params object[] partitionKeyComponents)
    {
        ...
        {
            query = query.Where(x => EF.Property<string>(x, "PartitionKey") == partitionKey);
        }
    ...
    public virtual async ValueTask InsertAsync(TEntity entity, bool detach=true)
    {
    ...
    public virtual async ValueTask UpdateAsync(TEntity entity)
    {
        if (_repositoryContext.Entry(entity).State == EntityState.Detached)
        {
            ...
            // The value of shadow key property 'shadow-property' is unknown when attempting to save changes.
            ...
            var idProperty = entityEntry.Metadata
                .GetProperties()
                .SingleOrDefault(x => x.Name == "Id");

            if (idProperty != null)
            {
                var id = entityEntry.Properties.Single(x => x.Metadata == idProperty);
                id.IsModified = true;
            }

            var partitionKeyProperty = entityEntry.Metadata
                .GetProperties()
                .SingleOrDefault(x => x.Name == "PartitionKey");

            if (partitionKeyProperty != null)
            {
                var partitionKey = entityEntry.Properties.Single(x => x.Metadata == partitionKeyProperty);
                partitionKey.CurrentValue = _partitionKeyValueGenerator.Value(_appState, entity);
